...
We haven't covered databases yet, but if you have the ASP.NET Web Site Starter Site, you already have a database. You are going to need to locate and add database support for this to work.
...
On that page click the "Edit database configuration" button. In the Alias box, put the name of your connection -- this is the name that you used in the Web.config file, with the starter site template it is StarterSite. Click Apply. This returns you to the previous page, and the alias now shows up in the information there. 
...
////
Now, on your local machine, you have connection strings configured. These will be different on the server, and so you will have to do a few more configuration steps to get your application talking to the database.
...
- http://www.devcurry.com/2012/05/deploying-your-aspnet-app-on-appharbor.html
