...
. Remember the dice calculations? First, build a method that will return a dice roll (one die). Then, build a Dice class so that you can create multiple dice. Then build a second class that holds two dice and upon request will compute a new roll and return it as the sum of the two dice; this can be impelemented to require an object or as static methods and properties. It should have both of its dice be stored in private variables (or an array of 2). Develop a simple page that reports a dice roll, using your classes to perform the roll, every time the user requests a new roll (a simple Submit button is fine).
...
Dice and PairOfDice class from exercise 1 and your updated StatCalc.cs class from exercise 2.
+
...
Redo your prorotype to use this object -- create a static 
