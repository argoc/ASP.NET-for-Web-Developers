...
one per line. A word is defined to be a sequence of letters. Any punctuation in
the input that is not part of a word (i.e., apostrophes are parts of words if there is an s before and after them; semicolons, commas, quotes, and other punctuation are not parts of words) should be discarded. For example, if the user
